...
public class brackets extends mode{

	private configData data = new configData();
	private events plugin = events.getInstance();
	public brackets(boolean enabled, boolean started, boolean joinable, String id, String name, Material icon, List<Player> players, ArrayList<ItemStack> inv , HashMap<String, ItemStack>armor) {
	...
	{
		mode mode = plugin.getmodeManager().getMode("brackets");
	    if (((((Damageable) e.getEntity()).getHealth() - e.getFinalDamage()) <= 0) //Checks if the entity will die and if entity is player
